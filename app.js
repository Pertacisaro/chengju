...
async function showMockAnalysis() {
  ...
  analyzeButton.textContent = "拆解中";

  try {
    if (!isServerMode()) {
      throw new Error("需要通过本地服务打开网页，才能使用 DeepSeek API。当前先显示模拟拆解。");
    }

    currentAnalysis = await requestAiAnalysis(text);
    inputMessage.textContent = "DeepSeek 拆解完成。";
  } catch (error) {
    if (!isServerMode()) {
      currentAnalysis = createMockAnalysis(text);
      inputMessage.textContent = error.message || "AI 暂时不可用，已显示模拟拆解。";
    } else {
      currentAnalysis = null;
      inputMessage.textContent = error.message || "DeepSeek 拆解失败，请检查 API Key 或本地服务。";
    }
  } finally {
    analyzeButton.disabled = false;
    analyzeButton.textContent = "开始拆解";
  }
